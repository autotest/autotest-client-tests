...
class smatch(test.test):
    ...
    def execute(self, kernel = None, build_target = None, base_tree = None, \
        ...
        if kernel == None and base_tree == None:
            raise TestError("Test require at least one parameter {kernel | base_tree}")
        ...
        if not own_kernel:
            ...
            logging.info('Save original kernel config')
        ...
        if not own_kernel:
            logging.info('Restore original kernel config')
