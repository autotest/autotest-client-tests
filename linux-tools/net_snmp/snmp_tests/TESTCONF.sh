...
if [ "x$SNMP_TEST_HOST" = "x" ]; then
    ...
    SNMP_CONFIG_FILE="$SNMP_TMPDIR_REMOTE/snmpd.conf"
    SNMPTRAPD_CONFIG_FILE="$SNMP_TMPDIR_REMOTE/snmptrapd.conf"
    SNMP_SNMPTRAPD_LOG_FILE="$SNMP_TMPDIR_REMOTE/snmptrapd.log"
    SNMP_SNMPTRAPD_PID_FILE="$SNMP_TMPDIR_REMOTE/snmptrapd.pid"
    SNMP_SNMPD_PID_FILE="$SNMP_TMPDIR_REMOTE/snmpd.pid"
    SNMP_SNMPD_LOG_FILE="$SNMP_TMPDIR_REMOTE/snmpd.log"
fi

## Setup default flags and ports if not done
