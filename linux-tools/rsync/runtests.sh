...
do
    ...
    SELinux=OFF
    selstat=$(getenforce)
    if [ `echo $selstat |grep -iv "disabled"` ];then
       SELinux=ON
    fi
    if [ "$SELinux" == "ON" ]; then
        if [ "$test" ==  "xattrs.test" ] || [ "$test" ==  "xattrs-hlink.test" ]; then 
            grep ^+   $scratchdir/test.log  > $scratchdir/diff.tmp
            grep ^-   $scratchdir/test.log  >> $scratchdir/diff.tmp
            sed -i '/^+#/d' $scratchdir/diff.tmp
            sed -i '/^@@/d' $scratchdir/diff.tmp
            sed -i '/^+++/d' $scratchdir/diff.tmp
            sed -i '/^---/d' $scratchdir/diff.tmp
            sed -i '/^+$/d' $scratchdir/diff.tmp
            if [ `grep -cvi "user.rsync.security.selinux" $scratchdir/diff.tmp` -eq 0 ]; then 
	       result=0
            fi
        fi
    fi
