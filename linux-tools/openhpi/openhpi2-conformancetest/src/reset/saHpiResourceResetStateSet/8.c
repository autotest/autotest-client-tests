 ...
 * Description:   
 *   Call saHpiResourceResetStateSet to set reset state as
 *   SAHPI_RESET_ASSERT for resource which doesn't support it
                  ...
                  SaHpiRptEntryT rpt_entry, callback2_t func)
{
        ...
        if (rpt_entry.ResourceCapabilities & SAHPI_CAPABILITY_RESET) {
                ...
                if (val == SA_ERR_HPI_INVALID_CMD) {
                        printf
                            ("  Find the resource which doesn't support ASSERT!\n");
                        ret = SAF_TEST_PASS;
                        goto out;
                }
                if (val != SA_OK) {
