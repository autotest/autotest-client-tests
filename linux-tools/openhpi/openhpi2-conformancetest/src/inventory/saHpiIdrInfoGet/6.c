		  ...
		  SaHpiRptEntryT report, callback2_t func)
{
	...
	if (hasInventoryCapability(&report)) {
		...
		//
		// Call saHpiIdrInfoGet() passing in a NULL pointer for IdrInfo.
		//

		status = saHpiIdrInfoGet(sessionId,
					 report.ResourceId,
					 SAHPI_DEFAULT_INVENTORY_ID, NULL);

		if (status == SA_ERR_HPI_INVALID_PARAMS) {
			retval = SAF_TEST_PASS;
		} else if (status == SA_ERR_HPI_NOT_PRESENT) {	// Is this a bug?
			retval = SAF_TEST_PASS;
		} else {
