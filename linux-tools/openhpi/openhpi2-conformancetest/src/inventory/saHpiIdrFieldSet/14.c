...
/*************************************************************************
 *
 * Test setting a field with characters that are illegal for
 * the given DataType.   In this case, use BCDPLUS which does
 * not support letters.
 *
 *************************************************************************/
	      ...
	      SaHpiIdrIdT IdrId, SaHpiEntryIdT AreaId)
{
	...
	if (status == SA_ERR_HPI_INVALID_DATA) {
		...
		Field.Field.DataType = SAHPI_TL_TYPE_BCDPLUS;
