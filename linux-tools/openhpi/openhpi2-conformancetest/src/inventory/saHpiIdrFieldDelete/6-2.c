		...
		SaHpiIdrIdT IdrId, SaHpiEntryIdT AreaId)
{
	...
	status = saHpiIdrFieldDelete(sessionId,
				     resourceId,
				     IdrId, AreaId, INVALID_FIELD_ID);

	if (status == SA_ERR_HPI_NOT_PRESENT) {
		retval = SAF_TEST_PASS;
	} else {
		retval = SAF_TEST_FAIL;
		e_print(saHpiIdrFieldDelete, SA_ERR_HPI_NOT_PRESENT, status);
	}
...
/*************************************************************************
 *
 * Traverse the Areas in a IDR.  We only care about obtaining valid
 * id values to use in order to call saHpiIdrFieldDelete().
 *
 * Note that the below loop will only execute until we find an writable
