...
/*************************************************************************
 *
 * Test an invalid resource id.  Note that it is possible for other
 * error codes to be returned.  If that occurs, we will return NOTSUPPORT
 * since it is legal to return other error codes.  
 *
	     ...
	     SaHpiIdrIdT IdrId, SaHpiEntryIdT AreaId)
{
	...
	status =
	    saHpiIdrFieldAdd(sessionId, INVALID_RESOURCE_ID, IdrId, &Field);

	if (status == SA_ERR_HPI_INVALID_DATA) {
		retval = SAF_TEST_NOTSUPPORT;
	} else if (status == SA_ERR_HPI_OUT_OF_SPACE) {
		retval = SAF_TEST_NOTSUPPORT;
	} else if (status == SA_ERR_HPI_INVALID_RESOURCE) {
		retval = SAF_TEST_PASS;
	} else {
		retval = SAF_TEST_FAIL;
		e_print(saHpiIdrFieldAdd, SA_ERR_HPI_INVALID_RESOURCE, status);
	}
...
/*************************************************************************
 *
 * For Areas that we can write to, try adding a Field using an
 * invalid resource id.  If we can't find any Areas that we can write
 * to, we will return NOTSUPPORT.
 *
 * NOTE: Actually I'm not sure if this algorithm is the best approach.
 *       In other words, why care about using an IDR that we can write
 *       to when we are using a different resource id.  The only benefit
 *       is to obtain parameter values that are legal.
 *
 *************************************************************************/
