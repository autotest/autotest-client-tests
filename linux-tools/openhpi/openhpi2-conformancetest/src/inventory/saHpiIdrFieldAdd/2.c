...
/*************************************************************************
 *
 * Test an invalid session id.  Note that it is possible for other
	     ...
	     SaHpiIdrIdT IdrId, SaHpiEntryIdT AreaId)
{
	...
	status =
	    saHpiIdrFieldAdd(INVALID_SESSION_ID, resourceId, IdrId, &Field);

	if (status == SA_ERR_HPI_INVALID_DATA) {
		retval = SAF_TEST_NOTSUPPORT;
	} else if (status == SA_ERR_HPI_OUT_OF_SPACE) {
		retval = SAF_TEST_NOTSUPPORT;
	} else if (status == SA_ERR_HPI_INVALID_SESSION) {
		retval = SAF_TEST_PASS;
	} else {
		retval = SAF_TEST_FAIL;
		e_print(saHpiIdrFieldAdd, SA_ERR_HPI_INVALID_SESSION, status);
	}
 ...
 * invalid session id.  If we can't find any Areas that we can write
 ...
 *       to when we are using a different session id.  The only benefit
