...
/*************************************************************************
 *
 * Add a field to an area.  Verify that the ReadOnly flag is false.
 *
	     ...
	     SaHpiIdrIdT IdrId, SaHpiEntryIdT AreaId)
{
	...
	if (status == SA_ERR_HPI_INVALID_DATA) {
		...
		status = saHpiIdrFieldGet(sessionId, resourceId, IdrId,
					  AreaId,
		...
		while ((retval == SAF_TEST_UNKNOWN) &&
			...
			if (status == SA_ERR_HPI_NOT_PRESENT) {
				...
				if (response == SAF_TEST_PASS) {
					...
					retval = SAF_TEST_FAIL;
				} else if (response == SAF_TEST_UNRESOLVED) {
					retval = SAF_TEST_UNRESOLVED;
				}
			}
		}
