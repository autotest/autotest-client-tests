...
/*************************************************************************
 *
 * Test an invalid IDR.  Note that it is possible for other
	     ...
	     SaHpiIdrIdT IdrId, SaHpiEntryIdT AreaId)
{
	...
	status =
	    saHpiIdrFieldAdd(sessionId, resourceId, INVALID_INVENTORY_ID,
			     &Field);
	...
	if (status == SA_ERR_HPI_INVALID_DATA) {
		...
		e_print(saHpiIdrFieldAdd, SA_ERR_HPI_NOT_PRESENT, status);
	}
 ...
 * invalid IDR.  If we can't find any Areas that we can write
