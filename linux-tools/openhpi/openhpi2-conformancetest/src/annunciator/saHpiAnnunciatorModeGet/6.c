...
/*************************************************************************
 *
 * Test using a NULL pointer for the mode parameter.
 *
 *************************************************************************/
		    ...
		    SaHpiRdrT * rdr, SaHpiAnnunciatorRecT * annunRec)
{
	...
	status = saHpiAnnunciatorModeGet(sessionId, resourceId, a_num, NULL);

	if (status == SA_ERR_HPI_INVALID_PARAMS) {
		retval = SAF_TEST_PASS;
	} else {
		retval = SAF_TEST_FAIL;
		e_print(saHpiAnnunciatorModeGet, SA_ERR_HPI_INVALID_PARAMS,
			status);
	}
