		    ...
		    SaHpiRdrT * rdr, SaHpiAnnunciatorRecT * annunRec)
{
	...
	status = saHpiAnnunciatorDelete(sessionId, INVALID_RESOURCE_ID,
					a_num, entryId, SAHPI_INFORMATIONAL);

	if (status == SA_ERR_HPI_INVALID_RESOURCE) {
		retval = SAF_TEST_PASS;
	} else {
		retval = SAF_TEST_FAIL;
		e_print(saHpiAnnunciatorDelete, SA_ERR_HPI_INVALID_RESOURCE,
			status);
	}
