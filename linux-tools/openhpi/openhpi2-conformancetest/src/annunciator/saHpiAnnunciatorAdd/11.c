...
/*************************************************************************
 *
 * Add an announcement and verify that the AddedByUser field 
 * was set to true.
 *
 *************************************************************************/
		    ...
		    SaHpiRdrT * rdr, SaHpiAnnunciatorRecT * annunRec)
{
	...
	if (status == ANNUN_ERROR) {
		...
		info_announcement.AddedByUser = SAHPI_FALSE;
		...
		if (status == SA_ERR_HPI_OUT_OF_SPACE) {
			...
			if (info_announcement.AddedByUser) {
				retval = SAF_TEST_PASS;
			} else {
				retval = SAF_TEST_FAIL;
				m_print
				    ("saHpiAnnunciatorAdd failed to set AddedByUser to true!");
			}
