...
int Test_Resource(SaHpiSessionIdT session, SaHpiResourceIdT resource)
{
	...
	//  Call saHpiEventLogStateSet passing in a bad sessionId
	//
	status = saHpiEventLogStateSet(INVALID_SESSION_ID,
				       resource, EnabledState);
	if (status == SA_ERR_HPI_INVALID_SESSION)
		retval = SAF_TEST_PASS_AND_EXIT;
	else {
		e_print(saHpiEventLogStateSet, SA_ERR_HPI_INVALID_SESSION,
			status);
