...
int Test_Domain(SaHpiSessionIdT session_id)
{
	...
	val = saHpiEventLogStateGet(session_id, SAHPI_UNSPECIFIED_RESOURCE_ID,
				    &enable_new);
	if (val != SA_OK) {
		e_print(saHpiEventLogStateGet, SA_OK, val);
		ret = SAF_TEST_UNRESOLVED;
	...
	if (enable_new != SAHPI_FALSE) {
		e_print(saHpiEventLogStateGet, SAHPI_FALSE, enable_new);
		ret = SAF_TEST_FAIL;
	} else {
		ret = SAF_TEST_PASS;
	}
		  ...
		  callback2_t func)
{
	...
	if (rpt_entry.ResourceCapabilities & SAHPI_CAPABILITY_EVENT_LOG) {
		...
		val =
		    saHpiEventLogStateSet(session_id, resource_id, SAHPI_FALSE);
		if (val != SA_OK) {
			e_print(saHpiEventLogStateSet, SA_OK, val);
			ret = SAF_TEST_FAIL;
			goto out;
		...
		if (enable_new != SAHPI_FALSE) {
	      ...
	      out1:
		val =
		    saHpiEventLogStateSet(session_id, resource_id, enable_old);
	}
