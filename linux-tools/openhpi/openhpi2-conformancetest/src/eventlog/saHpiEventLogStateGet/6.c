...
int Test_Resource(SaHpiSessionIdT session, SaHpiResourceIdT resource)
{
	...
	//
	//  Call saHpiEventLogStateGet passing in a NULL 
	//  pointer for EnabledState
	//
	status = saHpiEventLogStateGet(session, resource, NULL);
	if (status == SA_ERR_HPI_INVALID_PARAMS)
		retval = SAF_TEST_PASS;
	else {
		e_print(saHpiEventLogStateGet, SA_ERR_HPI_INVALID_PARAMS,
			status);
