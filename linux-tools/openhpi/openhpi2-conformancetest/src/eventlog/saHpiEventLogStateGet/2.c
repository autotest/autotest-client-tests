...
int Test_Resource(SaHpiSessionIdT session, SaHpiResourceIdT resource)
{
	...
	SaHpiBoolT EnabledState;

	//  Call saHpiEventLogStateGet passing in a bad sessionId
	//
	status = saHpiEventLogStateGet(INVALID_SESSION_ID,
				       resource, &EnabledState);
	if (status == SA_ERR_HPI_INVALID_SESSION)
		retval = SAF_TEST_PASS_AND_EXIT;
	else {
		e_print(saHpiEventLogStateGet, SA_ERR_HPI_INVALID_SESSION,
			status);
