...
int Test_Resource(SaHpiSessionIdT session, SaHpiResourceIdT resource)
{
	...
	//
	//  Call saHpiEventLogEntryAdd passing in a bad SessionId
	//
	status = saHpiEventLogEntryAdd(INVALID_SESSION_ID, resource, &EvtEntry);
	if (status == SA_ERR_HPI_INVALID_SESSION) {
		retval = SAF_TEST_PASS;
	} else {
		e_print(saHpiEventLogEntryAdd, SA_ERR_HPI_INVALID_SESSION,
			status);
