 ...
 * from the DAT for all severities.  To adequately test this, six alarms will be
 ...
 * three unacknowledged alarms should be returned.
 * *******************************************************************************/
...
int Test_Domain(SaHpiSessionIdT session_id)
{
	...
	if (retval == SAF_TEST_UNKNOWN) {
		//
		//  Retrieve all of the un-acknowledged alarms from all 
		//  of the domains.
		//
		alarm.AlarmId = SAHPI_FIRST_ENTRY;
		status = SA_OK;
		while (status == SA_OK) {
			status = saHpiAlarmGetNext(session_id, SAHPI_ALL_SEVERITIES, SAHPI_TRUE,	//un-acknowledged only
						   &alarm);
			...
			if (status == SA_ERR_HPI_NOT_PRESENT) {
				...
				m_print
				    ("Function \"saHpiAlarmGetNext\" returned an acknowledged alarm instead!");
				retval = SAF_TEST_FAIL;
				break;
			} else if (isAlarm(alarm.AlarmId, ackAlarm)) {
				m_print
				    ("Function \"saHpiAlarmGetNext\" returned a user alarm that was supposed to be acknowledged!");
				retval = SAF_TEST_FAIL;
		...
		if (retval == SAF_TEST_UNKNOWN) {
			// 
			// If we get here, then we read the DAT without encountering an error.
			// We must be sure now that that we did encounter the three unacknowledged
			// alarms that we added.  
			//
			if (count == ALARM_COUNT) {
				retval = SAF_TEST_PASS;
			} else {
				m_print
				    ("Function \"saHpiAlarmGetNext\" did not return all of the user alarms!");
				retval = SAF_TEST_FAIL;
			}
		}
	}
	// Remove any and all user alarms that were added.
	//
