...
int Test_Domain(SaHpiSessionIdT session_id)
{
	...
	// Attempt to delete the Alarm with a bad SessionID
	status =
	    saHpiAlarmDelete(INVALID_SESSION_ID, SAHPI_ENTRY_UNSPECIFIED,
			     SAHPI_MINOR);
	if (status != SA_ERR_HPI_INVALID_SESSION) {
		e_print(saHpiAlarmDelete, SA_ERR_HPI_INVALID_SESSION, status);
