...
int Test_Domain(SaHpiSessionIdT session_id)
{
	...
	SaHpiBoolT NewAlarmAdd = SAHPI_FALSE;
	...
	} else			// alarm has been created and needs to be removed later
		NewAlarmAdd = SAHPI_TRUE;

	if (retval == SAF_TEST_UNKNOWN) {
		// When the Alarm is successfully set

		status = saHpiAlarmAcknowledge(session_id,
					       NewAlarm.AlarmId, SAHPI_MAJOR);
		if (status != SA_OK) {
	...
	if (retval == SAF_TEST_UNKNOWN) {
		...
		if (status != SA_OK) {
			// Unable to read a new alarm
			e_print(saHpiAlarmGet, SA_OK, status);
			retval = SAF_TEST_UNRESOLVED;
		} else {
			if (Alarm.Acknowledged == SAHPI_FALSE) {
				// Alarm failed to confirm acknowledgement
				m_print
				    ("Function \"saHpiAlarmAcknowledge\" failed to acknowledge the specific alarm with wrong severity!");
				retval = SAF_TEST_FAIL;
			} else
				retval = SAF_TEST_PASS;
		}
	}
	if (NewAlarmAdd != SAHPI_FALSE) {
		// Clean up
