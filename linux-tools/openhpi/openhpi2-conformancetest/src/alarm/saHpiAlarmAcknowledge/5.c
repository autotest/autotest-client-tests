...
int Test_Domain(SaHpiSessionIdT session_id)
{
	...
	NewAlarm.Acknowledged = SAHPI_TRUE;
	...
	} else			// alarm has been created and needs to be removed later
	{
		...
		if (status != SA_OK) {
			...
			if (Alarm.Acknowledged != SAHPI_FALSE) {
				...
				m_print
				    ("Function \"saHpiAlarmAdd\" created a alarm in an acknowledge condition!");
				retval = SAF_TEST_UNRESOLVED;
			}
		}
		...
		if (retval == SAF_TEST_UNKNOWN) {
			...
			if (status != SA_OK) {
				...
				if (Alarm.Acknowledged == SAHPI_FALSE) {
					...
					m_print
					    ("Function \"saHpiAlarmAcknowledge\" failed to acknowledge the alarm!");
					retval = SAF_TEST_FAIL;
				} else
