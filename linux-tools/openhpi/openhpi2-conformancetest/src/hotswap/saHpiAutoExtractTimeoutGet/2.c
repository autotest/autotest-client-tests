 ...
 *      saHpiAutoExtractTimeoutGet() returns SA_ERR_HPI_INVALID_SESSION.
 * Line:        P29-47:P29-49
 *    
 */
#include <stdio.h>
		  ...
		  SaHpiRptEntryT report, callback2_t func)
{
	...
	if (report.ResourceCapabilities & SAHPI_CAPABILITY_MANAGED_HOTSWAP) {
		...
		if (session == bad_ID)
			bad_ID++;
		//
		//  Call saHpiAutoExtractTimeoutGet passing in a bad SessionId
		//
		status = saHpiAutoExtractTimeoutGet(bad_ID,
						    report.ResourceId,
						    &Timeout);
