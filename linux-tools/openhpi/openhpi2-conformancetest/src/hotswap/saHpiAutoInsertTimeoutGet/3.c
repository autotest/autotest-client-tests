 ...
 * Description:
 *   Pass in a NULL pointer for Timeout.
 *   saHpiAutoInsertTimeoutGet() returns SA_ERR_HPI_INVALID_PARAMS.
 * Line:        P141-17:P141-17
 *    
 */
#include <stdio.h>
#include "saf_test.h"
...
int ForEachDomain(SaHpiSessionIdT session_id)
{
	...
	//
	//  Call saHpiAutoInsertTimeoutGet passing in a NULL Timeout pointer.
	//
	status = saHpiAutoInsertTimeoutGet(session_id, NULL);
