		  ...
		  SaHpiRptEntryT report, callback2_t func)
{
	...
	// If there exist in this implementation an instance of
	// a resource with Full Hot Swap Model, then this test should
	// be valid.
	if ((report.ResourceCapabilities & SAHPI_CAPABILITY_MANAGED_HOTSWAP) &&
	    (report.ResourceCapabilities & SAHPI_CAPABILITY_FRU)) {
		//
		//  Call saHpiResourceInactiveSet passing in a bad ResourceId
		//
		status = saHpiResourceInactiveSet(session, BAD_RESOURCE_ID);
