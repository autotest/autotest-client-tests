 ...
 * Description:
 *   For each Hotswapable resource which supports the full hot swap
 *   model, transition the resource into the extraction pending state
 *   and call the function to transition it to the active state.
 *   saHpiResourceInactiveSet() returns SA_OK.
 * Line:        P140-13:P140-13
 *    
 */
#include <stdio.h>
#include <unistd.h>
...
int TestCase_Inactivate(SaHpiSessionIdT session,
			...
			int *returnvalue, SaHpiBoolT * restore)
{
	...
	if (*returnvalue == SAF_TEST_UNKNOWN) {
		...
		if (status != SA_OK) {
			e_print(saHpiResourceInactiveSet, SA_OK, status);
		...
		if (retval == SAF_TEST_UNKNOWN) {
			...
			case SAHPI_HS_STATE_ACTIVE:
				Extract(session, report.ResourceId, &retval,
					&Restore);
				TestCase_Inactivate(session, report.ResourceId,
						    &retval, &Restore);
