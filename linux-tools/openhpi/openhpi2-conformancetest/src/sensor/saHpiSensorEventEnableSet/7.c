          ...
          if ((sensorCount < MAX_SENSORS) &&
              ...
              && (sensorRec->ThresholdDefn.
               ...
               if (status == SA_ERR_HPI_ENTITY_NOT_PRESENT) {
                    ...
                    if (status != SA_OK) {
                         retval = SAF_TEST_UNRESOLVED;
                         e_print(saHpiSensorEventMaskGet, SA_OK,
                              status);
 ...
 * UpMinor threshold to be equal to the LowMinor threshold.  Under normal 
 * circumstances, this would cause an event.  But since we have disabled
 * event generation, we should not get an event.
 *
 **********************************************************************************/
     ...
     if (status != SA_OK) {
          ...
          if (response != SAF_TEST_PASS) {
               ...
               if (status != SA_OK) {
                    ...
                    sensorData[sensorCount].enableEvents = SAHPI_FALSE;
                    sensorCount++;
               }
          }
     }
 ...
 * generation is already disabled, we can simply run the test.  If it is enabled,
      ...
      if (status != SA_OK) {
           ...
           if (status != SA_OK) {
                ...
                if (retval == SAF_TEST_PASS) {
                        sensorData[sensorCount-1].enableEvents = SAHPI_TRUE;
                } else {
                        status = saHpiSensorEventEnableSet(sessionId, resourceId,
