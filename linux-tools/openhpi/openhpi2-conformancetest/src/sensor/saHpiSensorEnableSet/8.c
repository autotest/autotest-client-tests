 ...
 * Description:   
 *   Disable a sensor, then verify that
 *   no events will generated for this sensor.
 * Line:        P86-23:P86-24
 */
...
void restoreSensors(SaHpiSessionIdT sessionId) {
        ...
        for (i = 0; i < sensorCount; i++) {
                ...
                status = saHpiSensorEnableSet(sessionId, sensorData[i].resourceId,
                                              sensorData[i].sensorNum, SAHPI_TRUE);
                if (status != SA_OK) {
 ...
 *      5) The UpMinor assert event must be set.
 *
 **********************************************************************************/
        ...
        if (rdr->RdrType == SAHPI_SENSOR_RDR) {
                ...
                if ((sensorCount < MAX_SENSORS) &&
                    ...
                    && (sensorRec->ThresholdDefn.
                        ...
                        status =
                            saHpiSensorEventMasksGet(sessionId, resourceId,
                                                     sensorRec->Num,
                                                     &assertMask, NULL);
         ...
         if (status == SA_ERR_HPI_TIMEOUT) {
              ...
              for (i = 0; i < sensorCount; i++) {
                  if ((sensorData[i].resourceId == event.Source) &&
                      ...
                      m_print("Resource %d; sensor %d generated an UpMinor threshold event!",
                               sensorData[i].resourceId, sensorNum);
                      break;
                  }
              }
         }
             ...
             SaHpiSensorRecT * sensorRec)
{
        ...
        if (status != SA_OK) {
                ...
                response = isReadingWithinMinorLimits(sessionId, resourceId,
                ...
                if (response != SAF_TEST_PASS) {
                        ...
                        if (status != SA_OK) {
                                ...
                                setThresholdValue(&newThresholds.UpMinor, &thresholds.LowMinor);
                                ...
                                if (status != SA_OK) {
                                     ...
                                     setThresholdValue(&newThresholds.UpMinor, &thresholds.UpMinor);
...
/**********************************************************************************
 *
 * Note that if event generation is already enabled, we can simply run the test.  
 * If it is disabled, we will enable it and then run the test.
 *
 **********************************************************************************/

int runSensorTest(SaHpiSessionIdT sessionId, SaHpiResourceIdT resourceId,
                  SaHpiSensorRecT * sensorRec)
{
        SaErrorT status;
        int retval = SAF_TEST_NOTSUPPORT;
        ...
        status = saHpiSensorEventEnableGet(sessionId, resourceId, sensorRec->Num, &enabled);
        if (status != SA_OK) {
                retval = SAF_TEST_UNRESOLVED;
                e_print(saHpiSensorEventEnableGet, SA_OK, status);
        } else if (enabled) {
                ...
                status = saHpiSensorEventEnableSet(sessionId, resourceId,
                                                   sensorRec->Num, SAHPI_TRUE);
                if (status != SA_OK) {
                        retval = SAF_TEST_UNRESOLVED;
                        e_print(saHpiSensorEventEnableGet, SA_OK, status);
...
int testSensor(SaHpiSessionIdT sessionId, SaHpiResourceIdT resourceId, SaHpiRdrT *rdr)
{
        ...
        status = saHpiSensorEnableGet(sessionId, resourceId, sensorRec->Num, &sensorEnabled);
        if (status != SA_OK) {
                retval = SAF_TEST_UNRESOLVED;
                e_print(saHpiSensorEnableGet, SA_OK, status);
        } else if (sensorEnabled) {
                retval = runSensorTest(sessionId, resourceId, sensorRec);
        }
