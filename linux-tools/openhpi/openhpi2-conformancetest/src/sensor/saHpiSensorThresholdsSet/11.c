...
SaHpiBoolT hasMin(SaHpiSensorRecT * sensorRec)
{
...
/********************************************************************************************
 *
 * Is it possible to get an underflow?  An underflow can only occur if
 * the min value is also the smallest value possible for the data type.
 *
 ********************************************************************************************/

SaHpiBoolT isUnderflow(SaHpiSensorReadingUnionT value,
		       SaHpiSensorReadingTypeT type)
{
	...
	case SAHPI_SENSOR_READING_TYPE_FLOAT64:
		return (value.SensorFloat64 == MIN_FLOAT64);
...
/********************************************************************************************
 *
 * Decrease the given value by a small amount.
 *
 ********************************************************************************************/

SaHpiSensorReadingUnionT decreaseValue(SaHpiSensorReadingUnionT value,
				       SaHpiSensorReadingTypeT type)
{
 ...
 * the min range error that we are seeking.
 *
 ********************************************************************************************/
	     ...
	     SaHpiResourceIdT resourceId, SaHpiRdrT rdr)
{
	...
	if (canTest(sessionId, resourceId, &rdr)) {
		...
		if (hasMin(sensorRec)) {
			...
			if (!isUnderflow(minValue, type)) {

				if (hasWriteThreshold
				    (sensorRec, SAHPI_STM_LOW_CRIT)) {

					initThresholds(&threshold, type);

					threshold.LowCritical.IsSupported =
					    SAHPI_TRUE;
					threshold.LowCritical.Value =
					    decreaseValue(minValue, type);
				    ...
				    if (hasWriteThreshold
					(sensorRec, SAHPI_STM_LOW_MAJOR)) {

					initThresholds(&threshold, type);

					threshold.LowMajor.IsSupported =
					    SAHPI_TRUE;
					threshold.LowMajor.Value =
					    decreaseValue(minValue, type);
				    ...
				    if (hasWriteThreshold
					(sensorRec, SAHPI_STM_LOW_MINOR)) {

					initThresholds(&threshold, type);

					threshold.LowMinor.IsSupported =
					    SAHPI_TRUE;
					threshold.LowMinor.Value =
					    decreaseValue(minValue, type);
