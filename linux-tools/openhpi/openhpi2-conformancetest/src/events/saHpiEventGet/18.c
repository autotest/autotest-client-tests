...
static void *timerThread(void *ptr) {
    ...
    while (runTimerThread) {
        ...
        timerCounter--;
        if (timerCounter == 0) {
            m_print("Close the session from a separate thread.");
...
int main() {
    ...
    if (error != SA_OK) {
        ...
        if (error != SA_OK) {
             ...
             * 2 seconds the session will be closed by a separate
             * thread.  If we can go 2 seconds without an unwanted
             * event, then we should get an INVALID_SESSION error code
             * returned. 
             */
