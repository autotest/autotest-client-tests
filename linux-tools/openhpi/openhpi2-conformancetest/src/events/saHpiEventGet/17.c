...
int main()
{
	...
	// wrong overflow reset
	// val = saHpiEventLogOverflowReset(session_id, SAHPI_UNSPECIFIED_RESOURCE_ID);
	...
	val = saHpiEventGet(session_id, SAHPI_TIMEOUT_IMMEDIATE, &event, &rdr,
			    &rpt_entry, NULL);
	...
	if (val != SA_OK || eqs == SAHPI_EVT_QUEUE_OVERFLOW) {
		e_print(saHpiEventGet,
			val == SA_OK && eqs != SAHPI_EVT_QUEUE_OVERFLOW, val);
		ret = SAF_TEST_FAIL;
	} else
		ret = SAF_TEST_PASS;
