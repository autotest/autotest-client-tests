...
int Test_Resource(SaHpiSessionIdT session)
{
	...
	status = saHpiSubscribe(session);
	if (status != SA_OK) {
		retval = SAF_TEST_UNRESOLVED;
		e_print(saHpiSubscribe, SA_OK, status);
	} else {
		//
		//  Call saHpiEventGet passing in a bad SessionId
		//
		status = saHpiEventGet(INVALID_SESSION_ID,
		...
		if (status == SA_ERR_HPI_INVALID_SESSION) {
			...
			e_print(saHpiEventGet, SA_ERR_HPI_INVALID_SESSION,
				status);
		}
		status = saHpiUnsubscribe(session);
	}
	return (retval);
}
