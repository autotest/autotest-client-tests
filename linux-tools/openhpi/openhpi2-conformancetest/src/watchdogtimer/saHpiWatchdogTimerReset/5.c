...
/*************************************************************************
 *
 * Test an invalid watchdog number.
 *
 *************************************************************************/
		       ...
		       SaHpiRdrT * rdr, SaHpiWatchdogRecT * watchdogRec)
{
	...
	//  Call saHpiWatchdogTimerReset passing with a bad watchdog number

	status =
	    saHpiWatchdogTimerReset(sessionId, resourceId, BAD_WATCHDOG_NUM);
	if (status == SA_ERR_HPI_NOT_PRESENT) {
		retval = SAF_TEST_PASS;
	} else {
		retval = SAF_TEST_FAIL;
		e_print(saHpiWatchdogTimerReset, SA_ERR_HPI_NOT_PRESENT,
			status);
	}
