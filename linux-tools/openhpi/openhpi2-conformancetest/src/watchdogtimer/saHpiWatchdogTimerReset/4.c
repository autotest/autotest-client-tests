		  ...
		  SaHpiRptEntryT report, callback2_t func)
{
	...
	if (!hasWatchdogCapability(&report)) {
		...
		//  Call saHpiWatchdogTimerReset on a resource which 
		//  does not support watchdog timers.

		status =
		    saHpiWatchdogTimerReset(sessionId, report.ResourceId, 0);
		if (status == SA_ERR_HPI_CAPABILITY) {
			retval = SAF_TEST_PASS;
		} else {
			retval = SAF_TEST_FAIL;
			e_print(saHpiWatchdogTimerReset, SA_ERR_HPI_CAPABILITY,
				status);
		}
	}
