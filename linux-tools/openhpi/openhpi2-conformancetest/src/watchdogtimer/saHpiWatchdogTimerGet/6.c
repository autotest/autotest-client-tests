		       ...
		       SaHpiRdrT * rdr, SaHpiWatchdogRecT * watchdogRec)
{
	...
	//  Call saHpiWatchdogTimerGet passing in a NULL 
	//  pointer for Watchdog.

	status = saHpiWatchdogTimerGet(sessionId, resourceId, w_num, NULL);

	if (status == SA_ERR_HPI_INVALID_PARAMS) {
		retval = SAF_TEST_PASS;
	} else {
		retval = SAF_TEST_FAIL;
		e_print(saHpiWatchdogTimerGet, SA_ERR_HPI_INVALID_PARAMS,
			status);
	}
