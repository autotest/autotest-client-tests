		   ...
		   SaHpiRdrT * rdr, SaHpiCtrlRecT * ctrlRec)
{
	...
	SaHpiResourceIdT bad_resource_id = INVALID_RESOURCE_ID;

	if (bad_resource_id == resourceId) {
		bad_resource_id++;
	}

	status = saHpiControlSet(sessionId, bad_resource_id, ctrlNum,
	...
	if (status == SA_ERR_HPI_INVALID_RESOURCE) {
		...
		e_print(saHpiControlSet, SA_ERR_HPI_INVALID_RESOURCE, status);
	}
