...
/*************************************************************************
 *
 * Set a new value for the Discrete control.
 *
 *************************************************************************/

SaErrorT setDiscreteValue(SaHpiSessionIdT sessionId,
...
/*************************************************************************
 *
 * If we have an Discrete control that we can set, try setting the value.
 * Since we don't know which values are legal or not, let's just try
 * the default value.
 *
 *************************************************************************/
		   ...
		   SaHpiRdrT * rdr, SaHpiCtrlRecT * ctrlRec)
{
	...
	if (!ctrlRec->WriteOnly && isDiscreteControl(ctrlRec)
		...
		if (status != SA_OK) {
			...
			status =
			    setDiscreteValue(sessionId, resourceId, ctrlRec,
					     ctrlRec->TypeUnion.Discrete.
					     Default);
