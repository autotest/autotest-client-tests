...
gentestcert () {
    ...
    openssl dhparam -dsaparam -out $name-dh.pem 1024
    cat $name-dh.pem >> $name.pem
}
...
testserversec "$N" "$TEST" "$opts -s" "tcp4-l:$PORT,reuseaddr,fork,retry=1" "" "range=127.0.0.0:255.255.0.0" "tcp4:127.1.0.0:$PORT,bind=127.1.0.0" 4 tcp $PORT 0
fi ;; # Linux, NUMCOND
esac
PORT=$((PORT+1))
N=$((N+1))
